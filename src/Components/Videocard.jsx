...
function Videocard({ displayData, setDeleteResponse, insideCategory }) {
    ...
    return (
        <>
            ...
            <Card className='mt-3' draggable={true} onDragStart={e => dragStarted(e, displayData?.id)}>
                <Card.Img onClick={handleShow} height={'300px'} variant="top" src={displayData?.imageURL} />
                <Card.Body>
                    <Card.Title className='d-flex justify-content-center align-items-center'>
                        <p>{displayData?.caption}</p>
                        {!insideCategory && <button onClick={() => handleRemoveVideo(displayData?.id)} className='btn' style={{ border: 'none' }}><i className='fa-solid fa-trash text-danger'></i></button>}
                    </Card.Title>
                </Card.Body>
            </Card>
